...
public:
	 ...
	 * @param	icon 图标字符
	 */
    static void SetIcon(QPushButton* btn, int iconSize, QChar icon);//设置图标
    static void SavePlaylist(QStringList& playList);//保存播放列表
    static void GetPlaylist(QStringList& playList);//获取播放列表
    static void SavePlayVolume(double& nVolume);//保存播放音量
    static void GetPlayVolume(double& nVolume);//获取播放音量
